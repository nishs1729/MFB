ss = '''PMCA_density_presynaptic = 180
...
mm = '''
    Cai, + PMCA0' <-> PMCAi_1'      [>k_PMCA0_1, <k_PMCA1_0]
    PMCAi_1'       -> PMCAi_2'      [k_PMCA1_2]
    PMCAi_2'       -> PMCA0'        [k_PMCA2_0]'''


for nAZ in range(7,36):
    text = ss
    for naz in range(1, nAZ+1):
        text += '''\tPMCAi_1 {D_2D = 0}\n\tPMCAi_2 {D_2D = 0}\n'''.replace('PMCAi', f'PMCA{naz}')
        
    text += ss2
    for naz in range(1, nAZ+1):
        text += mm.replace('Cai', f'Ca{naz}').replace('PMCAi', f'PMCA{naz}') + '\n'
    text += ee
    print(text)

    with open(f'/home/nishant/lab/MFB/model_mcell/control_eachCa/pmca/pmca_AZ_{nAZ}.mdl', 'w+') as wf: wf.write(text)